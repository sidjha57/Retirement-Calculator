...
/**
 * Sidebar component representing the navigation menu.
    ...
    >
      ...
      <ul className="pt-4">
        {Object.keys(MENUS).map((title) => (
          <Link
            className="link"
            to={MENUS[title as MenuTitleType].link ?? '/home'}
            key={MENUS[title as MenuTitleType].id}
            onClick={() => {
              setSelectedID(MENUS[title as MenuTitleType].id)
            }}
          >
            <li
              className={`flex rounded-md p-2 cursor-pointer hover:bg-light-white text-gray-300 text-sm items-center gap-x-4 
              ${MENUS[title as MenuTitleType].gap ? 'mt-9' : 'mt-2'} ${
                MENUS[title as MenuTitleType].id === selectedID ? 'bg-light-white' : ''
              } `}
            >
              <div className="flex w-full justify-between">
                ...
                <span
                  className={`material-symbols-outlined ${MENUS[title as MenuTitleType].textColor}`}
                >
